...
New backend = new sibling class + new `match`/`case` arm in `initializeModels.py` or `src/factories/` + register names in argparse `choices` (`src/cli/parser.py`) and `modelsList()`/`modelsMap()` (`src/model/downloadModels.py`). **NEVER rewrite the CUDA path.** No ABC — convention only; each class implements `__call__(frame, ...)` + `handleModel()`.
...
- **Add a model** = TWO edits: weight mapping in `src/model/downloadModels.py` `modelsMap()` AND the flag `choices` in `src/cli/parser.py`. (`modelsList()` in `src/model/downloadModels.py` is the canonical method-name registry, reused by `--offline.`)
