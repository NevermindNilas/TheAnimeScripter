...
### Added
- **D-FINE object detection** ([Peterande/D-FINE](https://github.com/Peterande/D-FINE), Apache-2.0 code + weights — free for commercial use). New `--obj_detect_method` values `dfine_{small,medium,large}-tensorrt` and `dfine_{small,medium,large}-openvino`. D-FINE is a real-time DETR (HGNetv2 backbone + transformer decoder), NMS-free, and higher-accuracy than the incumbent YOLOv9-MIT (S ~48 / M ~52 / L ~54 COCO AP; DEIM-trained weights drop in on the identical graph for ~54.7–56.5). Chosen over RF-DETR (SOTA tier is the platform-locked PML-1.0 license), YOLO-NAS (non-commercial weights) and Ultralytics/YOLOv6/v7 (AGPL/GPL) — all disqualified for closed-source commercial use. COCO-trained ONNX (paired fp16/fp32, S/M/L) hosted on TAS-Models-Host.
  - Consumes the **RAW exported head**: single `images[1,3,640,640]` input, two outputs `pred_logits[1,300,80]` + `pred_boxes[1,300,4]` (cxcywh, normalized). Decode (`src/objectDetection/dfine.py`): sigmoid → top-k over flattened (query, class) → cxcywh→xyxy → de-normalize, matching the reference D-FINE post-processor. The single-input form keeps TAS's ORT feed dict and the TRT static-binding loop unchanged (the baked-postproc export's `orig_target_sizes` second input would break both).
  - Implemented as sibling backends `ObjectDetectionDFineTensorRT` / `ObjectDetectionDFineDML` (subclass the YOLOv9-MIT classes, override only decode + the two-output TRT binding; the DML-named class serves the OpenVINO provider and casts the input to the model dtype for fp16 ONNX). Preprocessing and drawing are reused verbatim since D-FINE preprocessing (resize-640 + /255 + RGB + NCHW) already matches TAS. The YOLOv9-MIT path is untouched. Verified end-to-end: TensorRT fp16 engine builds and annotates real video at ~43 FPS (720p, small); fp16↔fp32 decode parity confirmed on real frames.
  - **No DirectML** for D-FINE: its DETR decoder uses ops the DirectML EP cannot compile (fails at session init regardless of precision, even after replacing `GridSample`). TensorRT (NVIDIA) and OpenVINO (Intel/CPU) only. YOLOv9-MIT remains the DirectML option.
  - Maintainer ONNX export helper at `scripts/export_dfine_onnx.py` (run inside a D-FINE checkout; emits paired fp16 + fp32 per size via the RAW head + TAS-style fp16 conversion). `THIRD_PARTY_NOTICES.md` updated with D-FINE + YOLOv9-MIT attribution.
