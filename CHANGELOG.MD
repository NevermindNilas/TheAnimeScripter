...
- `--depth_method` values that are CLI-accepted but have no model wiring now raise instead of silently doing nothing. `initializeModels.depth()`'s `match` had no default case, so methods such as `large_v3`, `giant_v3` (and their `-directml`/`-tensorrt`/`-openvino` variants) and `og_video_small_v2-tensorrt` fell through, ran no model, and reported success with no output. Added a `case _:` that raises (`src/initializeModels.py`).
...
- Truncated model downloads are detected and retried. The downloader committed files to the weights cache without verifying them against the advertised `content-length`, and `http.client.IncompleteRead` was not in the retry `except`. Added a size check that triggers the existing retry/cleanup path, widened the retry exceptions (`IncompleteRead`/`ConnectionError`/`TimeoutError`), and reset `downloadedBytes` per attempt (`src/utils/downloadModels.py`).
