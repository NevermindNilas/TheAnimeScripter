...
| Model | Speed | Quality | VRAM | Best For | Acceleration |
|-------|-------|---------|------|----------|--------------|
| **rife4.22** | ⚡⚡ | ⭐⭐⭐ | 4GB | General use | CUDA, TRT |
...
| **rife4.22-lite** | ⚡⚡⚡ | ⭐⭐ | 2GB | Low-end GPUs | CUDA, TRT |
| **rife4.6** | ⚡⚡⚡⚡ | ⭐⭐ | 4GB | Stable choice | CUDA, TRT |
| **rife4.15-lite** | ⚡⚡⚡ | ⭐⭐ | 2GB | Legacy lite | CUDA, TRT |
| **rife4.15** | ⚡⚡⚡ | ⭐⭐⭐ | 4GB | Legacy version | CUDA, TRT |
| **rife4.16-lite** | ⚡⚡⚡ | ⭐⭐ | 2GB | Legacy lite | CUDA |
| **rife4.17** | ⚡⚡ | ⭐⭐⭐ | 4GB | Legacy version | CUDA, TRT |
| **rife4.18** | ⚡⚡ | ⭐⭐⭐ | 4GB | Legacy version | CUDA, TRT |
| **rife4.20** | ⚡⚡ | ⭐⭐⭐ | 4GB | Legacy version | CUDA, TRT |
| **rife4.21** | ⚡⚡⚡ | ⭐⭐⭐ | 4GB | Legacy version | CUDA, TRT |
...
> **YOLOv9-MIT** (MIT license) and **D-FINE** (Apache-2.0) are both fully free for commercial use.
> YOLOv9 runs on `-directml` / `-openvino` / `-tensorrt`. D-FINE is a real-time DETR (NMS-free,
> higher accuracy) and runs on `-tensorrt` (NVIDIA) and `-openvino` (Intel/CPU) — **not DirectML**
> (its DETR decoder uses ops the DirectML runtime can't compile).
...
| **dfine_small-tensorrt** | ⚡⚡⚡ | ⭐⭐⭐⭐ | 2GB | Fast DETR (~48 AP) | TensorRT |
| **dfine_medium-tensorrt** | ⚡⚡ | ⭐⭐⭐⭐ | 4GB | Balanced DETR (~52 AP) | TensorRT |
| **dfine_large-tensorrt** | ⚡ | ⭐⭐⭐⭐⭐ | 6GB | Highest accuracy (~54 AP) | TensorRT |
| **dfine_{small,medium,large}-openvino** | ⚡⚡ | ⭐⭐⭐⭐ | — | D-FINE on Intel/CPU | OpenVINO |
...
| `--download_requirements` | optional str | None | **Download dependencies** for `windows-cuda`, `windows-lite`, `linux-cuda`, or `linux-lite`. With no value, prompts for full CUDA / TensorRT dependencies for GTX 16xx, RTX 20xx+, and newer NVIDIA GPUs, or lite dependencies for GTX 10xx, AMD, and Intel hardware. |
