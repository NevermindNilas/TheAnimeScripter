...
def _addSmoothDedupOptions(argParser):
    ...
    smoothDedupGroup.add_argument(
        ...
        type=int,
        default=6,
        help="Longest run of duplicates --smooth_dedup will interpolate across; longer holds are filled with copies instead. 0 removes the cap so every run is interpolated. Raising it costs inference time, not memory",
    )
...
def _addDepthOptions(argParser):
    ...
    depthGroup.add_argument(
        ...
        choices=[4, 8, 16, 32],
        default=32,
        help="Temporal window for video depth methods. For video_*_v3 and video_limbo*, "
...
def _addEncodingOptions(argParser):
    ...
    encodeMethods = [
        ...
        "jpeg",
        "nvenc_h264_nelux",
        "slow_nvenc_h264_nelux",
        "nvenc_h265_nelux",
        "slow_nvenc_h265_nelux",
