...
def depth_scale_shift_normalization(depth, low_percent=2, high_percent=98):
    ...
    min_value = torch.from_numpy(np.percentile(a=depth_, q=low_percent, axis=1)).to(
        depth
    )[..., None, None, None]
    max_value = torch.from_numpy(np.percentile(a=depth_, q=high_percent, axis=1)).to(
        depth
    )[..., None, None, None]
...
def create_point_cloud(depth_map, camera_matrix, extrinsic_matrix):
    ...
    homogeneous_camera_coords = depth_map_reshaped * np.dstack(
    ...
    homogeneous_world_coords = np.dot(homogeneous_camera_coords, extrinsic_matrix.T)

    # Divide by the fourth coordinate (homogeneous normalization)
    point_cloud = (
        homogeneous_world_coords[:, :, :3] / homogeneous_world_coords[:, :, 3:]
    )
...
def Disparity_Normalization(disparity):
    ...
    normalized_disparity = (
        (disparity - min_value) / (max_value - min_value + 1e-5) - 0.5
    ) * 2
