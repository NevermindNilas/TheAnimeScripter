...
class DepthTensorRT:
    ...
    def handleModels(self):
        ...
        outputHeightAndWidth = f"{self.height}x{self.width}"

        engineName = f"{self.depth_method}_{outputHeightAndWidth}_{enginePrecision}.engine"

        if not os.path.exists(modelPath.replace(".onnx", engineName)):
            ...
            self.engine = SaveEngine(
                self.engine, modelPath.replace(".onnx", engineName)
            )
...
class DepthDirectML:
    def __init__(
        ...
        self.height = height
        self.fps = fps
        self.half = half = False  # force setting to fp 32
        self.inpoint = inpoint
        self.outpoint = outpoint
        self.encode_method = encode_method
        self.depth_method = depth_method
