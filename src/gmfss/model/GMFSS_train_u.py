...
class Model:
    def __init__(self, local_rank=-1):
        ...
        self.gan_loss = GANLoss(gan_type='vanilla').to(device)
    ...
    def inference(self, img0, img1, timestep, simple_color_aug):
        with torch.no_grad():
            ...
            rife = F.interpolate(rife, scale_factor = 0.5, mode="bilinear", align_corners=False)

            img0, img1, rife = simple_color_aug.augment(img0), simple_color_aug.augment(img1), simple_color_aug.augment(rife)
        ...
        with torch.autocast(device_type='cuda'):
            ...
            merged = self.fusionnet(torch.cat([I1t, rife, I2t], dim=1), torch.cat([feat1t1, feat2t1], dim=1), torch.cat([feat1t2, feat2t2], dim=1), torch.cat([feat1t3, feat2t3], dim=1))
    ...
    def update(self, imgs, gt, learning_rate=0, training=True, timestep=0.5, step=0, spe=1136):
        ...
        with torch.autocast(device_type='cuda'):
            ...
            loss_gan = self.gan_loss(self.net_d(torch.clamp(merged, 0, 1)), True, is_disc=False)
        ...
        fake_d_pred = self.net_d(torch.clamp(merged, 0, 1).float().detach().clone())
        ...
        return merged, torch.cat((flow01, flow10), 1), metric0, metric1, loss_l1, loss_lpips, loss_gan
