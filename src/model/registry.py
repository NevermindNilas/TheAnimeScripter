"""Static model registry data without network or runtime startup imports."""
...
def modelsList() -> list[str]:
    return [
        ...
        "hurrdeblur-mps",
        "dehalo-mps",
        # ROCm (-rocm) mirrors the CUDA .pth weights; the runtime strips the
        # suffix before modelsMap like it does for -mps, so no modelsMap arm
        # is needed. TensorRT/Maxine/NVENC have no ROCm counterpart (NVIDIA-only).
        "shufflecugan-rocm",
        "cyte-rocm",
        "adore-rocm",
        "span-rocm",
...
def modelsMap(
    ...
    match model:
        ...
        case "limbo-tensorrt" | "limbo-directml":
            if half:
                return "Limbo_504x280_fp16.onnx"
            else:
                return "Limbo_504x280_fp32.onnx"

        case "limbo_43-tensorrt" | "limbo_43-directml":
            if half:
