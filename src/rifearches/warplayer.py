...
def warp(tenInput, tenFlow):
    ...
    if tenGrid is None or tenGrid.size() != tenFlow.size():
        ...
        tenHorizontal = torch.linspace(-1.0, 1.0, W, device=device).view(1, 1, 1, W).expand(N, -1, H, -1)
        tenVertical = torch.linspace(-1.0, 1.0, H, device=device).view(1, 1, H, 1).expand(N, -1, -1, W)
