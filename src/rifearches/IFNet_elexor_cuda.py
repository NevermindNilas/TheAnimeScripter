    ...
    def forward(self, img0, img1, timestep):
        ...
        for i in range(6):
            if flow is None:
                ...
                if self.ensemble:
                    f_, m_ = self.blocks[i](
                        torch.cat((img1, img0, self.f1, self.f0, 1 - timestep), 1),
                        self.ph,
                        self.pw,
                        None,
                        scale=self.scale_list[i],
                ...
                wf0 = warp(self.f0, flow[:, :2], self.tenFlow, self.backWarp)
                wf1 = warp(self.f1, flow[:, 2:4], self.tenFlow, self.backWarp)
                fd, m0 = self.blocks[i](
                ...
                if self.ensemble:
                    fd_, m_ = self.blocks[i](
                        torch.cat(
                            (warped_img1, warped_img0, wf1, wf0, 1 - timestep, -mask),
                            1,
                        ),
                        self.ph,
                        self.pw,
