...
class LineThinner(nn.Module):
    ...
    def _sobelX(self, luma: torch.Tensor, offset: float) -> torch.Tensor:
        ...
        gridLeft = (
            torch.stack([gridX - dx, gridY], dim=-1).unsqueeze(0).expand(B, -1, -1, -1)
        )
        left = F.grid_sample(
            luma, gridLeft, mode="bilinear", padding_mode="border", align_corners=True
        )
        ...
        right = F.grid_sample(
            luma, gridRight, mode="bilinear", padding_mode="border", align_corners=True
        )
    ...
    def _sobelY(self, sobelXY: torch.Tensor, offset: float) -> torch.Tensor:
        ...
        gridTop = (
            torch.stack([gridX, gridY - dy], dim=-1).unsqueeze(0).expand(B, -1, -1, -1)
        )
        top = F.grid_sample(
            sobelXY, gridTop, mode="bilinear", padding_mode="border", align_corners=True
        )
    ...
    def _gaussianBlur(self, edge_map: torch.Tensor, sigma: float) -> torch.Tensor:
        ...
        padded = F.pad(
            edge_map, (kernelSize // 2, kernelSize // 2, 0, 0), mode="replicate"
        )
        blurred = F.conv2d(padded, kernel_h, groups=edge_map.shape[1])

        kernel_v = kernel.view(1, 1, kernelSize, 1)
        padded = F.pad(
            blurred, (0, 0, kernelSize // 2, kernelSize // 2), mode="replicate"
        )
    ...
    def _applyWarp(
        ...
        for _ in range(self.iterations):
            ...
            dd = (
                dn_normalized
                * torch.tensor([dx.item(), dy.item()], device=device, dtype=image.dtype)
                * relstr
            )
        ...
        return F.grid_sample(
            image, pos, mode="bilinear", padding_mode="border", align_corners=True
        )
    ...
    def forward(self, image: torch.Tensor) -> torch.Tensor:
        ...
        if config["luma_scale"] < 1.0:
            image_scaled = F.interpolate(
                image, size=(luma_h, luma_w), mode="bilinear", align_corners=True
            )
        ...
        if self.variant == "veryfast":
            luma = F.interpolate(
                luma, size=(sobel_h, sobel_w), mode="bilinear", align_corners=True
            )
...
class LineThin:
    def __init__(
        ...
        self.model = LineThinner(variant=variant).to(
