...
class RifeROCm:
    """
    AMD ROCm (HIP) RIFE interpolator. Eager, no CUDA graphs or custom streams.

    Mirrors RifeMPS's eager path but stays on the torch.cuda (HIP) device and
    returns GPU tensors like RifeCuda. Shares .pth weights with the CUDA path:
    the "-rocm" suffix is stripped before resolving filenames and archs.
    """
    ...
    def __init__(
        ...
        self.height = height
        self.interpolateMethod = interpolateMethod
        self.baseMethod = interpolateMethod.replace("-rocm", "")
        self.ensemble = ensemble
        self.interpolateFactor = interpolateFactor
        self.dynamicScale = dynamicScale
        self.staticStep = staticStep
    ...
    def handleModel(self):
        if ADOBE:
            progressState.update(
                {"status": f"Loading ROCm RIFE model: {self.interpolateMethod}..."}
            )
        ...
        IFNet = importRifeArch(self.baseMethod, "v1", half=self.half)
